...
M.basic = {
	...
	-- double-j to leave insert mode (same idea as `<Esc>`, respects 'timeoutlen')
	{ 'jj', '<Esc>', mode = 'i', desc = 'Normal mode' },

	-- double-q: close buffer (Snacks keeps window layout; stock `qq` records a macro in `q`)
	{
		'qq',
		function()
...
M.snacks = function()
	...
	return {
		...
		-- Cmd+E: directory tree (Snacks explorer); `<D-` works in GUI/terminals that forward Cmd
		{ '<D-e>', S.explorer, desc = 'file explorer' },
